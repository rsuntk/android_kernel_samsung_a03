...
static void zram_set_bhandle(struct zram *zram, u32 index,
		unsigned long long blk_idx, unsigned int offset, unsigned int size)
{
	zram->table[index].bhandle = (blk_idx << (PAGE_SHIFT * 2)) | (offset << PAGE_SHIFT) | size;
}
#endif
		...
		struct zwbs *zwbs, unsigned long blk_idx, bool ppr)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (size == PAGE_SIZE)
			size = 0;
		zram_set_bhandle(zram, index, blk_idx, offset, size);
		zram_slot_unlock(zram, index);
...
static int read_comp_from_bdev(struct zram *zram, struct bio_vec *bvec,
			unsigned long long handle, struct bio *parent, bool ppr);
...
int zram_prefetch_entry(unsigned long index)
{
	...
	zram_slot_unlock(zram, index);
	if (read_comp_from_bdev(zram, NULL, bhandle, NULL, true) < 0)
		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	for (index = 0; index < nr_pages; index++) {
		...
		zram_set_flag(zram, index, ZRAM_WB);
#ifdef CONFIG_ZRAM_LRU_WRITEBACK
		zram_set_bhandle(zram, index, blk_idx, 0, 0);
#else
				...
				unsigned int blk_idx, int nr_pages)
{
	...
	while (idx < nr_pages) {
		...
		bhandle = zram_get_bhandle(zram, index);
		if ((bhandle >> (PAGE_SHIFT * 2)) != blk_idx + idx ||
			((bhandle >> PAGE_SHIFT) & (PAGE_SIZE - 1)) != offset ||
			(size == PAGE_SIZE && (bhandle & (PAGE_SIZE - 1)) != 0) ||
			(size != PAGE_SIZE && (bhandle & (PAGE_SIZE - 1)) != size)) {
			zram_slot_unlock(zram, index);
			goto next;
		}
...
static void zram_free_page(struct zram *zram, size_t index)
{
	...
	if (zram_test_flag(zram, index, ZRAM_WB)) {
		...
		free_block_bdev(zram, bhandle >> (PAGE_SHIFT * 2), ppr);
#else
		zram_clear_flag(zram, index, ZRAM_WB);
				...
				struct bio *bio, bool partial_io)
{
	...
	if (zram_test_flag(zram, index, ZRAM_WB)) {
		...
		bhandle = zram_get_bhandle(zram, index);
		blk_idx = bhandle >> (PAGE_SHIFT * 2);
		if (((bhandle & (PAGE_SIZE - 1)) != 0) || ppr) {
			zram_set_flag(zram, index, ZRAM_READ_BDEV);
			zram_inc_wb_table(zram, blk_idx);
			zram_slot_unlock(zram, index);
			ret = read_comp_from_bdev(zram, &bvec, bhandle, bio, ppr);
			if (ret < 0)
